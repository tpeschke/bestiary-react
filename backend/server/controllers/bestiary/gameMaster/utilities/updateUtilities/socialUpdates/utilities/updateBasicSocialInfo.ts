...
export default async function updateBasicSocialInfo(beastID: number, socialInfo: NonspecificSocialInfo) {
    ...
    set socialSkulls = $2, socialEpValue = $3, socialRole = $4, socialSecondary = $5, capacity = $6, atk_conf = $7
    where id = $1`

    const { socialSkulls, socialRawEpValue, socialRole, socialSecondary, capacity, attackInfo } = socialInfo

    return query(sqlQuery, [beastID, socialSkulls, socialRawEpValue, socialRole, socialSecondary, capacity.strength, attackInfo[BONFIRE]])
}