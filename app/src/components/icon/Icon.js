...
export default function Icon({ iconName, tooltip, color, iconSize, margin, htmlTooltip, float, onClick }) {
    ...
    if (styling === WARNING) {
        return _jsx("p", { className: 'warning', children: "(!)" });
    }
    if (htmlTooltip) {
        const { component, id } = htmlTooltip;
        return (_jsxs(_Fragment, { children: [_jsx("i", { onClick: onClick, "data-tooltip-id": id, className: styling }), _jsx(Tooltip, { id: id, children: _jsx("div", { children: component }) })] }));
    }
    return _jsx("i", { onClick: onClick, "data-tooltip-id": "my-tooltip", "data-tooltip-content": tooltip, className: styling });
}
