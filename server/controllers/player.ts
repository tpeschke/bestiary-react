...
export async function addPlayerNotes(request: noteRequest, response: Response) {
    ...
    if (user && noteId) {
        const result = await databaseConnection.user.notes.update(noteId, notes).catch((error: Error) => sendErrorForward('update beast notes', error, response))[0]
        checkForContentTypeBeforeSending(response, result)
    } else if (user && user.patreon) {
        const count = await databaseConnection.user.notes.number(user.id).catch((error: Error) => sendErrorForward('check user note count', error, response))[0]
        ...
        if (isAboveDefaultNumberOfNotes || isAboveNumberOfNotesForPatrons) {
            ...
            const result = databaseConnection.user.notes.add(beastId, user.id, notes).catch((error: Error) => sendErrorForward('save beast notes', error, response))[0]
            checkForContentTypeBeforeSending(response, result)
        }
    } else {
